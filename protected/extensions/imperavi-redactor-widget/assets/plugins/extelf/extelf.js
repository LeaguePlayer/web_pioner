...
RedactorPlugins.extelf = {
    init: function()
    {
        ...
        this.opts=$.extend({
            ...
            modal_elf:String() +
            '<section id="redactor_modal_content">' +
                '<label>' + RLANG.elurl + '</label>' +
                '<input type="text" id="redactor_elfurl" class="redactor_input" /><br/>' +
                '<div>' +
                    '<textarea  id="redactor_elfaslink" style="overflow:auto;width:534px;height:60px;"></textarea>' +
                    '<button id="redactor_insaslink"  class="redactor_modal_btn">'+RLANG.insaslink+'</button>'+
                '</div><br/>'+
                '<div>' +
                    '<textarea id="redactor_elfasimg" style="overflow:auto;width:534px;height:60px;"></textarea>' +
                    '<button id="redactor_insasimg" class="redactor_modal_btn">'+RLANG.insasimg+'</button>'+
                '</div><br/>'+
    ...
    *Тут мы вызываем модальное окно имперави*/
        obj.modalInit(RLANG.fm, obj.opts.modal_elf, 600, $.proxy(function() {
